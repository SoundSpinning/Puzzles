// get input image from hyperlink
inputPic = document.querySelector('img.raw');
...
} else {console.log("no new image found!");}
...
window.onload = function init() {
	...
	// when true the JS polyfill at bottom of .HTML will make it work
	if (isTouchScreen()) {
...
function spriteImg(image) {
	...
	for (var i = 0; i < nRows; i++) {
		for(var j = 0; j < nCols; j++){
			...
			// this is required to make touch polyfill to work
...
function dragEnter(target, event) {
	...
	// prevent dropping on a tile with children
